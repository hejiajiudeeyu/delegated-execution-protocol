...
定义 agent 生态中的委托执行协议，包括角色、对象模型、授权、合约、结果验证、信任模型、版本兼容与扩展边界。
...
发布形式：npm 包（`@delexec/contracts`），供 client 和 platform 仓库作为上游依赖引用。
...
| 来源路径 | 说明 |
| :--- | :--- |
| `packages/caller-controller-core/` | caller 侧核心逻辑 |
| `packages/responder-runtime-core/` | responder 侧运行时核心 |
| `packages/transports/` | 传输适配器（local、relay-http、email） |
| `packages/sqlite-store/` | 客户端侧本地存储 |
| `apps/caller-controller/` | Caller Controller 服务 |
| `apps/responder-controller/` | Responder Controller 服务 |
| `apps/ops/` | 统一 Ops CLI |
| `apps/ops-console/` | 用户控制台 |
| 客户端侧 unit / integration 测试 | 跟随源码 |
...
| e2e | platform-selfhost | 需要全栈环境，通过发布镜像拉起各组件 |

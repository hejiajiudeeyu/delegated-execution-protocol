Integration Playbook v0.1（买家/卖家/平台）
...
- 买家端如何发起请求、跟踪状态、验收结果。
- 卖家端如何接收请求、校验执行、回传结果。
- 协议控制面如何将目录稳定地“传给”买家与卖家（控制面分发）。

本文档描述的是 `Remote Subagent Protocol` 的接入方式，以及当前仓库参考实现的联调路径；它不要求未来所有实现共享同一个集中式托管中继。
...
1. 买家拉取目录，选定 `seller_id + subagent_id`。
2. 买家生成 `request_id` 与任务合约。
3. 买家调用 `POST /v1/tokens/task` 申请短期代币。
4. Buyer调用`POST /v1/requests/{request_id}/delivery-meta`获取单次投递元数据（`task_delivery + result_delivery + verify`）。
5. 当前实现：Buyer通过`POST /controller/requests/{request_id}/dispatch`把任务信封发送到`L0本地传输`。
6. 当前实现：Seller 通过 `POST /controller/inbox/pull` 拉取任务，解析并发并继续后续处理。
7. 卖家通过服务端上报ACK（已接单，可附ETA）。
8. 卖家执行任务，生成结果包并签名。
9. 当前实现：Seller 通过 `Transport Adapter` 选择相同的 `request_id` 语义回传结果包，由 Buyer Controller 接收。
10. 当前实现：卖方在结果确定后额外向平台写入 `COMPLETED` 或 `FAILED` 事件，作为控制面初始，不替代结果回传。
11. 买家轮询服务端请求事件与结果，按 `request_id` 验签+验签模式（含错误结果包）。
12. 买家/卖家分别调用 `POST /v1/metrics/events` 上报最小指标。

## 2. 买家端详细流程

## 2.1 启动前准备
- 先完成用户注册（`POST /v1/users/register`）并获取API Key（默认`buyer`范围）。
- 配置平台基本URL与认证信息（仅控制面API）。
- 当前本地参考实现中，`ops-console`通过本地密码解锁`~/.delexec/secrets.enc.json`；Buyer API key、seller key、transport Secrets不再读取默认浏览器存储。
...
- 买家确定目标子代理后，读取目录边境中的`template_ref`字段（语义绑定键）。
...
- 买方代理可以根据此自动构造合约中的`task.input`和`task.output_schema`。
...
- `result_delivery` 是卖家回传结果的绑定路由；当前实现支持`email|local|relay_http`，工件`platform_inbox`。
- 发送任务请求：
  - 统一信封：包含 `request_id / seller_id / subagent_id / Payload / thread_hint`
- 本地状态更新：`已创建 -> 已发送`
...
- 若未收到ACK：
  ...
  - 可触发一次重试或切换备选卖家
- ACK 后状态迁移：`接收 -> ACKED`
- 同一事件完成物流观察到卖方写入的`COMPLETED/FAILED`状态；这些事件仅用于控制面启动与辅助排障，并不直接决定买方本地终状态。
...
- 状态迁移：
  - v0.1 不实现 `RUNNING/PROGRESS` 细节事件；买家控制面通常观察到 `ACKED` 后等待传输结果，必要时可额外查看 `COMPLETED/FAILED` 出发事件
  - `status=ok` 且验证通过：买家本地终态为 `SUCCEEDED`
  - 协议验证不通过：`UNVERIFIED` 或 `FAILED(RESULT_*)`
  - `status=error` 且校验通过：买方本地终态通常映射为 `FAILED`，将 `error` 反馈给买方代理作为后续决策输入
  - 超过 `hard_timeout_s`：`TIMED_OUT`

## 2.5.1 超时确认策略（买家控制器）
- `soft_timeout_s`到达时默认询问买家代理是否继续等待（`timeout_confirmation_mode=ask_by_default`）。
- `hard_timeout_s` 到达且未收到 `continue_wait=true` 时，买方控制器自动终态为 `TIMED_OUT`。
- `TIMED_OUT`最多表示买家侧停止等待与轮询，不代表队列卖家进程一定被杀掉。

## 2.5.2 买家代理轮询控制器（内部接口）
...
## 3.1 启动前准备
- 先完成用户主体注册并领取API Key（默认买家角色）。
- 若该运行时要承载远程子代理，则通过 `POST /v1/catalog/subagents` 提交 onboarding 草案；卖家与子代理都需管理员渠道后才会进入公开目录。
- seller本地运行时可先启动；但仅当`seller.review_status=approved`、`subagent.review_status=approved`且双方`status=enabled`时，才可被买家发现和远程调用。
- seller侧调用平台接口前，需通过 `API key + seller scope + 资源归属(owner_user_id->seller_id->subagent_id)`鉴权。
- 当前联调模式内部外部通道；卖家通过`POST /controller/inbox/pull`从`L0本地运输`拉取任务。
...
7. ACK：调用`POST /v1/requests/{request_id}/ack`，最小上报`seller_id/subagent_id`，任选附加`eta_hint_s`。
...
12. 完成状态放置：卖方在结果包确定后调用 `POST /v1/requests/{request_id}/events` 写入 `COMPLETED` 或 `FAILED`。

## 3.2.1 审核测试
- 管理员可通过 `POST /v1/admin/subagents/{subagent_id}/review-tests` 发起隐藏审核测试。
...
## 3.6 卖家信息变更（L0后）
- **可修改字段**：`display_name`、`description`、`eta_hint`、`seller_public_key_pem`、`status`。
- **变更方式**：通过表单重新提交变更项 + 平台CLI审核导入（复用现有导入流程）。
- **变更变更**：触发双键窗口（见`../spec/architecture.md`第5.6节），买家验签自动兼容。
- **不可自主变更字段**：`subagent_id`、`seller_id`、`capability`、`supported_task_types`（需平台审核后由管理员变更）。
- **生效时间**：导入完成后立即生效，买家下次目录查询即可获取最新信息。
...
1. 用户完成主体注册并获取API Key（默认买家）。
2. 用户通过表单提交远程子代理信息与能力说明（填写 `owner_user_id`）。
3. 平台管理员基于模板维护子代理并关联`seller_id`。
4. 平台管理员通过CLI审核并导入；导入成功后激活该用户的卖家角色能力。
5. 买家通过查询接口获取可用目录入口。

模板文件：
- `docs/templates/catalog-subagent.template.json`
- `docs/templates/catalog-subagents.import.template.ndjson`
- 能力声明模板：`docs/templates/subagents/{subagent_id}/`（参见`../spec/architecture.md` §4.5）

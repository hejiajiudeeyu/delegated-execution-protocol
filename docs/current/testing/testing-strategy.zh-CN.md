...
- E2E 与兼容性测试优先使用独立进程边界，不直接导入各服务的 server factory
- 未来跨仓验收的启动约定见 [Cross-Repo 兼容性测试](/Users/hejiajiudeeyu/Documents/Projects/remote-hotline-protocol/docs/current/testing/cross-repo-compatibility-testing.md)
...
- E2E：平台 + Caller Controller + Responder Controller 的端到端场景，按独立进程 / HTTP 边界运行
...
- 流程图（步骤覆盖与编号节点）：`../diagrams/user-remote-hotline-call-flow.md`
- 规范文档（断言口径）：`../spec/architecture.md`、`../spec/platform-api-v0.1.md`、`../guides/integration-playbook.md`

## 4. 模拟/运行时策略

`tests/mocks/` 下保留以下替身定义（当前为工件，集成和 E2E 测试直接使用真实内存态服务实例）：
...
- Responder 通过 `inbox/pull` 拉取并 ACK 本地传输消息
- 平台继续只承担控制面职责
...
- 行为：启动 `docker-compose.yml`，等待三端健康检查，执行最小成功路径，再自动 `down`
- 目标：验证“真实进程 + 网络 + 端口映射 + 服务组合”无基础阻塞
- 严格模式语义：Docker 不可用时直接失败，避免 CI 中出现“绕过”的假阳性
...
并覆盖“错误结果包可被Caller验收并反馈”的路径。

当前E2E额外已验证：
- Responder 最终验签使用预绑定信任根，不信任结果包自带公钥
- `delivery-meta` 与令牌声明的 `request_id/responder_id/hotline_id/caller_id` 绑定

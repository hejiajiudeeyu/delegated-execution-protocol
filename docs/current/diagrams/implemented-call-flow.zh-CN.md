...
- Caller Agent（本地代理）通过 Caller Controller 编排协议调用
...
- Remote Hotline 在当前已实现基线中执行 Responder Runtime 内部挂载的执行器/工作流程/代码设施

```mermaid
sequenceDiagram
    autonumber
    participant BU as "Caller User"
    participant BA as "Caller Agent (Local Agent)"
    ...
    P-->>BC: candidate hotlines
    BC-->>BA: candidates + template_ref + signer key

    BA->>BC: select responder_id + hotline_id

# User -> Remote Hotline Call Flow (L0 Baseline + Future Notes)

## 关键澄清（与规范文档对齐）

### B 阶段：目录返回什么，Caller 才能充分筛选

- 规范锚点：`../spec/platform-api-v0.1.md` §3.1.1（Caller 筛选最小字段集）。
...
- Caller 读取目录项中的 `template_ref`，再调用：
...
- `template_ref` 是语义绑定键（可为路径样式或版本化标识）；Caller 不直接读取仓库路径。
...
- 该接口不属于平台对外 API。
- 当前参考实现里，`GET /controller/hotlines` 的 HTTP 响应本身就是 `Caller Controller -> Caller Agent/调用方` 的“返回候选列表”步骤；图中应显式画出，避免误读为 Caller 未收到查询结果就直接做选择。

### 平台事件与最终结果真值

- 当前最小实现中，Responder 会向平台写入 `ACKED`、`COMPLETED`、`FAILED` 等控制面事件。
- Caller 轮询平台事件主要用于观测 ACK/完成态和辅助超时管理。
- Caller 的最终验收真值仍来自 transport 回传的 `result_package` 及本地验签/校验，而不是平台事件本身。
...
- 当前（MVP）仅使用 Pull：Caller Controller 主动 `GET /v1/requests/{request_id}/events` 观察 `ACKED` 与 Responder 写入的 `COMPLETED/FAILED` 完成态事件。
- Pull 的主要影响：实现简单、无需 Caller 暴露回调入口；代价是轮询 QPS 与可见性延迟。
...
- 队列拒绝：超出并发/资源阈值时返回 `EXEC_QUEUE_FULL`（含 `retry_after_s`），Caller 可换候选或延迟重试。
- 观测指标：`queue_depth`、`queue_wait_ms_p95`、`run_ms_p95`、`reject_rate`。

## 失败分支最小处置表

- `PLATFORM_*`（目录、token、delivery-meta 阶段）  
  责任：Platform/Caller Controller。处置：短重试 + 熔断降级（缓存目录/备用策略）。
- `DELIVERY_*`（发信或回信阶段）  
  责任：对应侧 Controller + Transport Adapter。处置：同 `request_id` 幂等重发，最多 3 次退避。
- `AUTH_*`（introspect/claims 不通过）  
  责任：Responder Controller。处置：拒单并返回标准错误包，不进入执行。
- `CONTRACT_*`（合约字段/版本/超时/任务类型）  
  责任：Responder Controller（入站校验）与 Caller Agent（修正合约）。处置：拒单或改单重提。
- `EXEC_*`（执行超时/内部异常）  
  责任：Responder Agent（执行）+ Responder Controller（回包）。处置：返回 `retryable`，买方按策略重试。
- `RESULT_*`（签名/Schema 不通过）  
  责任：Caller Controller（机械校验）。处置：标记 `UNVERIFIED` 或 `FAILED`，不计成功。
- 语义验收失败（非协议错误）  
  责任：Caller Agent。处置：`FAILED`；人工复核属于后续流程。
- Caller 本地超时（`hard_timeout`）  
  责任：Caller Controller。处置：停止本地等待并标记 `TIMED_OUT`；不保证远端执行被 kill。

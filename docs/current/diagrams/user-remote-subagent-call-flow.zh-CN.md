用户 -> 远程子代理呼叫流程（L0 基线 + 未来注释）

> 英文版：[user-remote-subagent-call-flow.md](user-remote-subagent-call-flow.md)
> 说明：中文文档为准。
...
- 规范要点：`../spec/platform-api-v0.1.md` §3.1.1（Buyer筛选最小字段集）。
...
- `template_ref` 是语义绑定键（用于路径样式或版本化标识）；Buyer 不直接读取仓库路径。

### D阶段：Delivery Meta从哪来

- 规范节点：`../spec/platform-api-v0.1.md` §6.1。
- 买家在 `POST /v1/tokens/task` 成功后调用：
...
- 买家代理在超时询问时通过内部接口写入决策：
  - `POST /controller/requests/{request_id}/timeout-decision`（`continue_wait=true/false`）
- 该接口不属于平台外部API。
- 当前参考实现里，`GET /controller/catalog/subagents` 的 HTTP 响应本身就是 `Buyer Controller -> Buyer Agent/调用方` 的“返回候选列表”步骤；比喻应显式画出，避免误读为 Buyer 未收到查询结果就直接做选择。

### 平台事件与最终结果真值

- 当前最小实现中，卖家会向平台写入 `ACKED`、`COMPLETED`、`FAILED` 等控制面事件。
- 买家轮询平台事件主要用于启动ACK/完成状态和辅助超时管理。
- 买家的最终验收真值仍来自运输回传的`result_package`及本地验签/验货，而不是平台事件本身。

### 通信层实现（MCP 只是通道实现之一）

- `Controller` 与通信通道之间只依赖统一的 `Transport Adapter` 接口。
- 举例来说`Buyer Transport Adapter / Seller Transport Adapter / Delivery Channel`表示抽象分层，不绑定邮件或MCP。
- 当前已确认的通信方式至少包含4类：`L0本地传输`、`Email MCP`、`SMTP/API电子邮件桥接`、`HTTP/Webhook通道`。
- 因此不是Agent直接调MCP；Agent只和Controller交互，Controller再调用具体通信适配器。
- 如果某个运行模式选择Email，只是 `Delivery Channel` 的一个实例，不应该反向定义整张图的对象模型。

### 卖家鉴权来源与权限管理

- 本图仅保留“卖家调平台前有鉴权闸门”的最小语义。
...
- `A`：买家目标接收与任务编排（User -> Agent -> Controller）
- `B`：目录检索/搜索与候选获取
- `C`：候选决策、模板拉取、合同草案准备
- `D`：授权与投递元数据准备（`token` + `delivery-meta`）
- `E`：请求投递（Buyer侧发送到Seller侧）
- `F`：卖家入站鉴权与基础校验（含内省）
- `G`：卖家决策、执行、结果回传（含ACK/执行/回信）
- `H`：买家赞赏与终态上报
- `I`：卖家用户人工介入（可选）
...
- 当前（MVP）仅使用 Pull：买家控制器主动`GET /v1/requests/{request_id}/events` 观察`ACKED` 与卖家写入的`COMPLETED/FAILED` 完成状态事件。
- Pull 的主要影响：实现简单、无需买家调用回调入口；代价是轮询 QPS 和可见性延迟。
- Push暂不纳入当前实现，仅作为后续演进项（高并发/低延迟场景再引入）。
- Buyer Agent 与 Buyer Controller 之间默认采用内部轮询接口（`GET /controller/requests/{request_id}`）。

## 买家/卖家/平台状态（简版）

- 买家请求状态（L0）：`CREATED -> DISCOVERING -> CONTRACT_PREPARED -> AUTHORIZED -> SENT -> ACKED -> (SUCCEEDED | FAILED | UNVERIFIED | TIMED_OUT)`
- 卖家请求状态：`RECEIVED -> AUTH_CHECKING -> CONTRACT_CHECKING -> (REJECTED | QUEUED) -> RUNNING -> (RESULT_PACKED | ERROR_PACKED) -> REPLIED -> DONE`
...
- `PLATFORM_*`（目录、代币、交付元阶段）  
  责任：平台/买家控制器。消耗：短重试 + 熔断降级（服务器目录/备用策略）。
- `DELIVERY_*`（发信或回信阶段）  
  责任：对应侧Controller + Transport Adapter。支出：同`request_id` 幂等重发，最多3次退避。
- `AUTH_*`（内省/声明不通过）  
  责任：Seller Controller。支出：拒单并返回标准错误包，无法进入执行。
- `CONTRACT_*`（标注字段/版本/超时/任务类型）  
  责任：卖方控制者（入站核查）与买方代理（修改合约）。 履行：拒单或改单重提。
- `EXEC_*`（执行超时/内部异常）  
  责任：Seller Agent（执行）+ Seller Controller（回包）。 回应：返回 `retryable`，应答按策略重试。
- `RESULT_*`（签名/架构不通过）  
  责任：Buyer Controller（机械验证）。付出：标记“UNVERIFIED”或“FAILED”，不计成功。
- 协商验收失败（非协议错误）  
  责任：买家代理。支出：`FAILED`；人工复核属于后续流程。
- 买家本地超时（`hard_timeout`）  
  责任：Buyer Controller。付出：停止本地等待并标记`TIMED_OUT`；不保证终端执行被kill。
